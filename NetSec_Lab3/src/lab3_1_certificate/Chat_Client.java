...
public class Chat_Client {
	...
	public void start() {
		try {
			...
			ins=ins+" "+enForm+" "+hashForm+" "+dayForm;
			System.out.println("----------------------------------");
			pw.println(ins);
			/*
			 ...
			 */
			while (true) {
				System.out.println("a.send message to chat(input a)  b.get the online usernames(input b) c.LOCK the system");
				System.out.println("----------------------------------");
				...
				case "a":
					...
					if (infoType.equals("M")) {
						...
						if (type.equals("1")) {
							...
							lists = RSA.dealEnMes(keys, Pkey);
							System.out.println(lists);
							pw.println(type + " " + lists.get(0) + " " + username);
							/*
							 * 输入信息并加密
							 */
						 ...
						 */
						ArrayList<String> list = new ArrayList<>();
						list = RSA.dealEnMes(ciphertext, Pkey);
						// key=list.get(0);//获取特殊密钥，进行传输
						String ds = list.get(0);
						for (int i = 1; i < list.size(); i++) {
							ds = ds + "BENZ" + list.get(i);
						}
//						System.out.println(ds);
						pw.println("DS "+username+" "+ds);
						Thread.sleep(500);
...
//						break;
					 ...
					 */
						...
						// Thread.sleep(500);
						System.out.println("please input the file type you want :T. txt  J.jpg/bmp  L.LOCK the system");
						String type = scanner.nextLine();// 获取加密的方式
						if(type.equals("L")){
							LOCK();
						}
...
//							out.close();
	...
	private void setName(Scanner scan) throws Exception {
		...
		while (true) {
			...
			MD5 md=new MD5();
			hashpassword=md.digest(password);
			if (name.trim().equals("")) {
			...
			try {
				...
				while ((msgString = br.readLine()) != null) {
					 ...
					 */
						...
						String time =strs[5];
						Certificate cer=new Certificate(strs[1]+" "+strs[2], algForm, hashForm, time);
...
//						System.out.println(msgString);
						String info[] = msgString.split(" ");
						String keyss = info[1].replaceAll("BENZ", " ");
						String[] keys = keyss.split(" ");
						ArrayList<String> list = new ArrayList<>();
						for (int i = 0; i < keys.length; i++) {
//							System.out.println(keys[i]);
...
//						key = RSA.dealDeMes(list, Prkey);
						DSmes = RSA.dealDeMes(list, Prkey);
						System.out.println("Digital signature is "+DSmes);
					}
					/*
					 * Caesar secret key
					 */
...
//							System.out.println(keys[i]);
					 ...
					 */
						...
						if(!mes.equals(DSmes)){
							System.out.println("The cipher is different from the DS");
						}
	...
	/**
	 * 该函数实现：在任意时刻LOCK后，进行系统暂停服务，并且只有当用户输入的用户名以及密码正确后才可以恢复运行
	 * 
	 * 密码信息的确认需要执行MD5算法确认
	 * @param username
	 * @param password
	...
	public static void LOCK() throws InterruptedException{
		MD5 md = new MD5();
		Scanner scanner=new Scanner(System.in);
		System.out.println("Please input your username and password to unlock the system");
		System.out.println("Username:");
		String username=scanner.nextLine();
		System.out.println("Passwrod");
		String password=scanner.nextLine();
		String hashValue = md.digest(password);// 1.获取hash value
		System.out.println("The hash value of password is "+hashValue);
		if(hashValue.equals(hashpassword)){
			System.out.println("The system unlocked");
		}
		//第一次输入密码错误
		else
		{
